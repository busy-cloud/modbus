...
func (d *Device) Open() (err error) {
	...
	err, d.options = product.LoadConfig[Options](d.ProductId, "modbus")
	if err != nil {
		return err
	}
	...
	//添加计划任务
	if d.options.Crontab != "" {
		job, err := cron.Crontab(d.options.Crontab, fn)
		if err != nil {
			return err
		}
		d.jobs = append(d.jobs, job)
	}
	if d.options.Interval > 0 {
		job, err := cron.Interval(int64(d.options.Interval), fn)
		if err != nil {
			return err
		}
		d.jobs = append(d.jobs, job)
...
func (d *Device) Poll() (map[string]any, error) {
	if d.options == nil {
		return nil, errors.New("options not exist")
	}

	values := map[string]any{}
	for _, p := range d.options.Pollers {
		buf, err := d.master.Read(d.Station.Slave, p.Code, p.Address, p.Length)
		if err != nil {
			return nil, err
		}
		//解析
		err = p.Parse(d.options.Mapper, buf, values)
		if err != nil {
			return nil, err
		}
	}
...
func (d *Device) Get(key string) (any, error) {
	if d.options == nil || d.options.Mapper == nil {
		return nil, errors.New("mappers not exist")
	}

	pt, code, addr, size := d.options.Mapper.Lookup(key)
	if pt == nil {
...
func (d *Device) Set(key string, value any) error {
	...
	pt, code, addr, _ := d.options.Mapper.Lookup(key)
	if pt == nil {
